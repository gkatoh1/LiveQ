...
function ModerationPanel({ eventId }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel('admin_mod_panel')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'admin_notifications', filter: `event_id=eq.${eventId}` }, 
        ...
        <div key={alert.id} className="bg-red-900/20 border border-red-500/50 p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in slide-in-from-top-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
                <span className="font-bold text-red-300 truncate">{alert.nickname}</span>
                <span className="text-xs text-red-500/70 font-mono">ID:{alert.user_id.slice(0,4)}</span>
            </div>
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <button onClick={() => handleDismiss(alert.id)} className="flex-1 md:flex-none px-4 py-3 md:py-2 rounded border border-zinc-600 text-zinc-400 hover:bg-zinc-800 text-sm font-bold">無視</button>
            <button onClick={() => handleBan(alert.user_id, alert.id)} className="flex-1 md:flex-none px-4 py-3 md:py-2 rounded bg-red-600 text-white font-bold hover:bg-red-500 shadow-lg text-sm">BAN実行</button>
          </div>
        </div>
      ))}
    </div>
...
function ChatManager({ eventId }) {
  ...
  useEffect(() => {
    loadMsgs()
    const ch = supabase.channel('admin_chat_mgr').on('postgres_changes', { event: '*', schema: 'public', table: 'messages', filter: `event_id=eq.${eventId}` }, loadMsgs).subscribe()
    return () => supabase.removeChannel(ch)
  }, [eventId])

  const loadMsgs = async () => {
    const { data } = await supabase.from('messages').select('*').eq('event_id', eventId).order('created_at', { ascending: false }).limit(50)
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl md:text-2xl font-bold text-blue-400">💬 チャット管理</h2>
        <button onClick={clearAll} className="bg-red-900/30 text-red-300 border border-red-800 px-3 py-1 rounded text-xs hover:bg-red-900">履歴全消去</button>
      </div>
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 h-[60vh] overflow-y-auto">
        {msgs.map(m => (
          <div key={m.id} className="p-3 border-b border-zinc-800 flex justify-between items-start hover:bg-zinc-800/50">
            <div className="min-w-0 pr-2">
              <span className="text-xs text-zinc-500 font-bold block mb-0.5">{m.nickname}</span>
              <span className="text-zinc-300 text-sm break-all">{m.content}</span>
            </div>
            <button onClick={() => deleteMsg(m.id)} className="text-zinc-500 hover:text-red-500 p-2">✕</button>
          </div>
        ))}
...
function QuestionManager({ eventId }) {
  ...
  useEffect(() => {
    loadQs()
    const ch = supabase.channel('admin_qs_mgr').on('postgres_changes', { event: '*', schema: 'public', table: 'questions', filter: `event_id=eq.${eventId}` }, loadQs).subscribe()
    return () => supabase.removeChannel(ch)
  }, [eventId])

  const loadQs = async () => {
    const { data } = await supabase.from('questions').select('*').eq('event_id', eventId).order('created_at', { ascending: false })
    if(data) setQs(data)
  }
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl md:text-2xl font-bold text-green-400">❓ 質問管理</h2>
        <button onClick={clearAll} className="bg-red-900/30 text-red-300 border border-red-800 px-3 py-1 rounded text-xs hover:bg-red-900">全消去</button>
      </div>
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 h-[60vh] overflow-y-auto">
        {qs.map(q => (
          <div key={q.id} className="p-3 border-b border-zinc-800 flex justify-between items-start hover:bg-zinc-800/50">
            <div className="min-w-0 pr-2">
            ...
            <button onClick={() => deleteQ(q.id)} className="text-zinc-500 hover:text-red-500 p-2">✕</button>
          </div>
        ))}
...
function BannedUsersList({ eventId }) {
  ...
  const fetchBanned = async () => { 
      const { data } = await supabase.from('profiles').select('*').eq('is_banned', true).eq('event_id', eventId)
  ...
  const executeManualBan = async () => {
      ...
      if (!targets || targets.length === 0) {
          setStatus('❌ ユーザーが見つかりません')
          return
      }

      if (!confirm(`"${manualName}" に一致するユーザーが ${targets.length} 人見つかりました。全員BANしますか？`)) {
          setStatus('')
          return
      }
      ...
      setStatus(`✅ ${targets.length} 人をBANしました`)
      setManualName('')
      fetchBanned()
  }
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center">
        <h2 className="text-xl md:text-2xl font-bold text-zinc-400">🚫 BANリスト</h2>
        <button onClick={fetchBanned} className="text-sm text-blue-400 underline">更新</button>
      </div>

      <div className="bg-red-900/10 border border-red-900/50 p-4 rounded-xl">
          <label className="text-xs font-bold text-red-400 uppercase tracking-widest mb-2 block">手動BAN (Manual Ban)</label>
          <div className="flex gap-2">
              <input 
                 value={manualName}
                 onChange={e => setManualName(e.target.value)}
                 className="flex-1 bg-black border border-red-900/30 rounded-lg px-4 py-2 text-white placeholder-red-900/30 focus:outline-none focus:border-red-500"
          ...
          {status && <p className="text-sm font-bold mt-2 text-red-300 animate-pulse">{status}</p>}
      </div>
      ...
      <div className="space-y-2">
          ...
          {bannedUsers.map(user => (
            <div key={user.id} className="bg-zinc-900 p-4 rounded-xl border border-zinc-700 flex justify-between items-center">
              <div>
                <p className="font-bold text-white">{user.nickname || '名無し'}</p>
                <p className="text-[10px] text-zinc-500 font-mono">ID: {user.id}</p>
              </div>
              <button onClick={() => unbanUser(user.id)} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded text-sm font-bold">解除</button>
            </div>
          ))}
      </div>
    </div>
      ...
      <div className="bg-zinc-900 p-4 rounded-xl mb-6 border border-zinc-800 shadow-md">
        ...
        {opts.map((o,i)=>(
          <div key={i} className="flex gap-2 mb-2">
            <input value={o} onChange={e=>handleOpt(i,e.target.value)} placeholder={`選択肢 ${i+1}`} className="flex-1 bg-black p-3 rounded-lg border border-zinc-700 text-white"/>
            {opts.length>2 && <button onClick={()=>setOpts(opts.filter((_,x)=>x!==i))} className="text-zinc-500 px-3 text-xl">×</button>}
          </div>
        ))}
        <div className="flex gap-3 mt-4">
            <button onClick={()=>setOpts([...opts,''])} className="flex-1 bg-zinc-800 text-zinc-300 font-bold py-3 rounded-lg border border-zinc-700">+ 選択肢</button>
            <button onClick={createPoll} className="flex-1 bg-blue-600 text-white font-bold py-3 rounded-lg shadow-lg active:scale-95 transition-transform">作成する</button>
        </div>
      </div>
        ...
        {polls.map(p=>(
            <div key={p.id} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 flex flex-col gap-4">
                ...
                <div className="flex gap-2 w-full">
                    <button onClick={()=>toggleEntry(p.id)} className={`flex-1 py-2 rounded-lg text-sm font-bold border ${event.entry_poll_id===p.id?'bg-yellow-500 border-yellow-500 text-black shadow-[0_0_10px_rgba(234,179,8,0.4)]':'bg-black border-zinc-700 text-zinc-400'}`}>
                        {event.entry_poll_id===p.id ? '★ 参加時ON' : '参加時'}
                    </button>
                    <button onClick={()=>toggleLive(p.id)} className={`flex-1 py-2 rounded-lg text-sm font-bold border ${event.active_poll_id===p.id?'bg-red-600 border-red-600 text-white shadow-[0_0_10px_rgba(220,38,38,0.6)] animate-pulse':'bg-black border-zinc-700 text-zinc-400'}`}>
                        {event.active_poll_id===p.id ? '● LIVE中' : 'LIVE'}
                    </button>
                    <button onClick={()=>deletePoll(p.id)} className="w-10 flex items-center justify-center rounded-lg bg-zinc-800 text-red-500 border border-zinc-700">🗑️</button>
                </div>
            </div>
        ))}
      </div>
...
export default function EventAdmin() {
  ...
  const handleLogin = (e) => {
      ...
      if (event && passwordInput === event.password) {
          setIsAuthenticated(true)
          sessionStorage.setItem(`admin_pass_${slug}`, passwordInput)
      } else {
          alert("パスワードが違います")
      }
  }
  ...
  const handleLogout = () => {
      ...
      sessionStorage.removeItem(`admin_pass_${slug}`)
  }

  const downloadQr = async () => {
      try {
        const url = `${window.location.origin}/${event.slug}`
        const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&data=${encodeURIComponent(url)}&format=jpg`
        const response = await fetch(qrUrl);
        const blob = await response.blob();
        const downloadLink = document.createElement('a');
        downloadLink.href = URL.createObjectURL(blob);
        downloadLink.download = `event-qr-${event.slug}.jpg`;
  ...
  const updateSettings = async (e) => {
    ...
    const { error } = await supabase.from('events').update({ password, question_limit: limit }).eq('id', event.id)
    if (error) alert("エラー: " + error.message)
    else { 
        alert("設定を保存しました")
        sessionStorage.setItem(`admin_pass_${slug}`, password)
        setEvent({...event, password, question_limit: limit})
    }
  }
      ...
      return (
          <div className="h-[100dvh] bg-black text-white flex flex-col items-center justify-center p-6">
              <div className="w-full max-w-sm bg-zinc-900 p-8 rounded-2xl border border-zinc-800 shadow-2xl">
                  <h1 className="text-2xl font-bold mb-2 text-center text-zinc-100">LiveQ <span className="text-blue-500 text-sm">/ {event.name}</span></h1>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-zinc-800 pb-6">
            <div className="w-full md:w-auto">
                <h1 className="text-2xl md:text-3xl font-bold mb-1 break-words">LiveQ <span className="text-blue-500 text-lg">/ Admin</span></h1>
                <p className="text-zinc-500 text-xs mb-2">{event.name}</p>
                <div className="flex flex-wrap gap-4 text-sm mt-2">
                    ...
                    <button onClick={downloadQr} className="text-green-400 hover:text-green-300 flex items-center gap-1">⬇️ QRコード</button>
                ...
                <div className="space-y-6 pb-20">
                    <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-lg">
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                            <button onClick={()=>toggleFeature('enable_chat')} className={`p-4 rounded-xl border flex-1 font-bold flex justify-between items-center transition-all ${event.enable_chat?'bg-green-900/20 border-green-500/50 text-green-400':'bg-black border-zinc-700 text-zinc-500'}`}>
                                <span>💬 チャット機能</span>
                                <span className={`text-xs px-2 py-1 rounded ${event.enable_chat?'bg-green-500 text-black':'bg-zinc-800 text-zinc-500'}`}>{event.enable_chat?'ON':'OFF'}</span>
                            </button>
                            <button onClick={()=>toggleFeature('enable_questions')} className={`p-4 rounded-xl border flex-1 font-bold flex justify-between items-center transition-all ${event.enable_questions?'bg-green-900/20 border-green-500/50 text-green-400':'bg-black border-zinc-700 text-zinc-500'}`}>
                                <span>❓ 質問機能</span>
                                <span className={`text-xs px-2 py-1 rounded ${event.enable_questions?'bg-green-500 text-black':'bg-zinc-800 text-zinc-500'}`}>{event.enable_questions?'ON':'OFF'}</span>
                            </button>
                        </div>
                    </div>
